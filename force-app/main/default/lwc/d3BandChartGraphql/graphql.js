// ABOUTME: Bundle-local Salesforce GraphQL (v2) record-query builder and normalizer for the d3BandChartGraphql bundle.
 ...
 * Used by both the structured record-query path (project the date, lower-bound,
 * upper-bound, and optional center-line fields, then shape the band client-side)
 * and the free-text `graphqlQuery` override. When `objectApiName` is omitted or
 * absent from the payload, the first object key under `data.uiapi.query` is used —
 * so an admin's free-text query is accepted regardless of the object it targets.
