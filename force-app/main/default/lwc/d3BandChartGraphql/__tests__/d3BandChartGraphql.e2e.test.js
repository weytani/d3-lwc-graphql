...
describe("c-d3-band-chart-graphql e2e", () => {
  ...
  describe("full lifecycle", () => {
    ...
    it("GraphQL self-fetch path: no recordCollection -> wire emits records -> full pipeline", async () => {
  ...
  describe("error recovery", () => {
    ...
    it("GraphQL wire errors -> error state -> component shows error", async () => {
      const element = await createChart({
        recordCollection: [],
        objectApiName: "Opportunity"
      });

      graphql.emitErrors([{ message: "INVALID_FIELD: no such column" }]);
      await flushPromises();
      ...
      expect(errorEl).toBeTruthy();
      expect(errorEl.textContent).toContain("INVALID_FIELD");
    });
  });
